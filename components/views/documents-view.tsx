...
import { FileText, Search, Download, Eye, Filter, Upload, Clock, FileCheck, Lock, File, FilePen, Folder, ChevronRight, Star } from 'lucide-react'
...
const STATUS_CONFIG: Record<string, { labelEn: string; labelAr: string; bg: string; text: string }> = {
  approved:      { labelEn: 'Approved',       labelAr: 'معتمد',           bg: 'bg-emerald-500/10', text: 'text-emerald-700 dark:text-emerald-400' },
  pending_review:{ labelEn: 'Pending Review', labelAr: 'قيد المراجعة',    bg: 'bg-amber-500/10',   text: 'text-amber-700 dark:text-amber-400'     },
  draft:         { labelEn: 'Draft',          labelAr: 'مسودة',           bg: 'bg-blue-500/10',    text: 'text-blue-700 dark:text-blue-400'        },
  classified:    { labelEn: 'Classified',     labelAr: 'سري',             bg: 'bg-red-500/10',     text: 'text-red-700 dark:text-red-400'          },
}

const CLASSIFICATION_CONFIG: Record<string, { css: string; labelAr: string }> = {
  Public:       { css: 'bg-slate-100 dark:bg-slate-800/60 text-slate-600 dark:text-slate-400', labelAr: 'عام'           },
  Internal:     { css: 'bg-blue-500/10 text-blue-700 dark:text-blue-400',                      labelAr: 'داخلي'         },
  Restricted:   { css: 'bg-amber-500/10 text-amber-700 dark:text-amber-400',                  labelAr: 'مقيد'           },
  'Top Secret': { css: 'bg-red-500/10 text-red-700 dark:text-red-400',                        labelAr: 'سري للغاية'    },
}
...
export function DocumentsView() {
  ...
  const FOLDERS = [
    { key: 'mous',          name: T.folderMous,       count: 47,  icon: FileCheck, color: 'text-amber-600 dark:text-amber-400',    bg: 'bg-amber-500/10'   },
    { key: 'minutes',       name: T.folderMinutes,    count: 124, icon: FilePen,   color: 'text-blue-600 dark:text-blue-400',      bg: 'bg-blue-500/10'    },
    { key: 'correspondence',name: T.folderCorr,       count: 89,  icon: FileText,  color: 'text-violet-600 dark:text-violet-400',  bg: 'bg-violet-500/10'  },
    { key: 'policy',        name: T.folderPolicy,     count: 31,  icon: File,      color: 'text-emerald-600 dark:text-emerald-400',bg: 'bg-emerald-500/10' },
    { key: 'classified',    name: T.folderClassified, count: 12,  icon: Lock,      color: 'text-red-600 dark:text-red-400',        bg: 'bg-red-500/10'     },
    { key: 'reports',       name: T.folderReports,    count: 63,  icon: Folder,    color: 'text-cyan-600 dark:text-cyan-400',      bg: 'bg-cyan-500/10'    },
  ]

  const STATS = [
    { label: T.totalDocs,       value: '366', icon: FileText,    color: 'text-blue-600 dark:text-blue-400',    bg: 'bg-blue-500/10'    },
    { label: T.pendingReview,   value: '18',  icon: Clock,       color: 'text-amber-600 dark:text-amber-400',  bg: 'bg-amber-500/10'   },
    { label: T.approvedMonth,   value: '41',  icon: FileCheck,   color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10' },
    { label: T.classifiedFiles, value: '12',  icon: Lock,        color: 'text-red-600 dark:text-red-400',      bg: 'bg-red-500/10'     },
  ]
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-5">
        ...
        <div className="space-y-1.5">
          <h3 className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest mb-3 px-1">{T.foldersHeading}</h3>
            ...
            return (
              ...
              >
                <div className={`shrink-0 p-1 rounded-lg ${active ? 'bg-white/15' : folder.bg}`}>
                  <Icon className={`w-3.5 h-3.5 ${active ? 'text-primary-foreground' : folder.color}`} />
                </div>
          ...
          {/* Storage */}
          <Card className="border-border/50 rounded-2xl mt-4 shadow-sm">
            <CardContent className="p-4 space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{T.storageUsed}</p>
                <span className="text-[10px] font-bold text-foreground">62%</span>
              </div>
              <Progress value={62} className="h-2 rounded-full" />
              <p className="text-[11px] text-muted-foreground">{T.storageDetail}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="xl:col-span-3 space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button className="flex items-center gap-1.5 text-[12px] text-muted-foreground hover:text-foreground border border-border/60 px-3.5 py-2 rounded-xl transition-all duration-150 hover:bg-muted/60 font-medium">
          ...
          {/* Table */}
          <Card className="border-border/50 overflow-hidden shadow-sm rounded-2xl">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/50 bg-muted/30">
                    <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-5 py-3">{T.thDocument}</th>
                    <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-4 py-3 hidden sm:table-cell">{T.thType}</th>
                    <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-4 py-3 hidden md:table-cell">{T.thClass}</th>
                    <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-4 py-3">{T.thStatus}</th>
                    <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-4 py-3 hidden lg:table-cell">{T.thModified}</th>
                    <th className="text-right text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-5 py-3">{T.thActions}</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((doc) => {
                    ...
                    return (
                      <tr
                        key={doc.id}
                        className="border-b border-border/30 hover:bg-muted/30 transition-colors duration-100 group"
                      >
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <div className="p-1.5 bg-muted/60 rounded-lg shrink-0">
                              <DocIcon className="w-3.5 h-3.5 text-muted-foreground" />
                            </div>
                            <div className="min-w-0">
                              <p className="text-[13px] font-semibold text-foreground truncate max-w-[200px]">{doc.name}</p>
                              <p className="text-[10px] text-muted-foreground">{doc.size} · v{doc.version}</p>
                            </div>
                            {doc.starred && <Star className="w-3 h-3 text-amber-400 fill-amber-400 shrink-0" />}
                          </div>
                        </td>
                        <td className="px-4 py-3.5 hidden sm:table-cell">
                          <span className="text-[12px] text-muted-foreground font-medium">{doc.type}</span>
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          <Badge className={`${status.bg} ${status.text} border-0 text-[10px] px-2.5 py-0.5 rounded-lg font-semibold`}>
                            {isRtl ? status.labelAr : status.labelEn}
                          </Badge>
                        </td>
                        <td className="px-4 py-3.5 hidden lg:table-cell">
                          <span className="text-[12px] text-muted-foreground">{doc.date}</span>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-1.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                            <button className="p-1.5 rounded-lg hover:bg-primary/10 hover:text-primary transition-colors" title={T.preview}>
                              <Eye className="w-3.5 h-3.5" />
                            </button>
                            <button className="p-1.5 rounded-lg hover:bg-primary/10 hover:text-primary transition-colors" title={T.download}>
              ...
              {filtered.length === 0 && (
                <div className="py-16 text-center">
                  ...
                  <p className="text-[13px] font-semibold text-muted-foreground">{T.noDocsTitle}</p>
                  <p className="text-[11px] text-muted-foreground/60 mt-1">{T.noDocsSubtitle}</p>
                </div>
              )}
            </div>
          </Card>
