...
import {
  ...
  Users2, Calendar, ChevronDown, ChevronUp,
...
const DECISIONS = [
  {
    ...
    members: [
      { name: 'K. Ibrahim', vote: 'for' }, { name: 'K. Belhaj', vote: 'for' },
      { name: 'H. Al-Idrissi', vote: 'against' }, { name: 'Y. Benali', vote: 'for' },
  ...
  },
  {
    ...
    members: [
      { name: 'A. Al-Mansouri', vote: 'for' }, { name: 'S. Al-Rashidi', vote: 'for' },
      { name: 'K. Ibrahim', vote: 'for' }, { name: 'N. Hassan', vote: 'for' },
  ...
  },
  {
    ...
    members: [
      { name: 'O. Nasser', vote: 'pending' }, { name: 'H. Al-Jubouri', vote: 'pending' },
      { name: 'S. Al-Tamimi', vote: 'pending' }, { name: 'R. Al-Bayati', vote: 'pending' },
    ],
  },
  {
    id: 4,
    ...
    members: [
      { name: 'A. Al-Mansouri', vote: 'against' }, { name: 'S. Al-Rashidi', vote: 'against' },
      { name: 'K. Ibrahim', vote: 'for' }, { name: 'N. Hassan', vote: 'against' },
      { name: 'F. Al-Shami', vote: 'abstain' },
    ],
  },
  {
    ...
    members: [
      { name: 'S. Al-Rashidi', vote: 'for' }, { name: 'R. Al-Dabbas', vote: 'for' },
      { name: 'L. Nassar', vote: 'for' }, { name: 'A. Awad', vote: 'for' },
    ],
  },
]

const STATUS_CONFIG: Record<string, { labelEn: string; labelAr: string; bg: string; text: string; icon: React.ElementType }> = {
  pending_vote: { labelEn: 'Pending Vote', labelAr: 'في انتظار التصويت', bg: 'bg-amber-500/10',   text: 'text-amber-700',   icon: Vote         },
  approved:     { labelEn: 'Approved',     labelAr: 'موافق عليه',        bg: 'bg-emerald-500/10', text: 'text-emerald-700', icon: CheckCircle2 },
  rejected:     { labelEn: 'Rejected',     labelAr: 'مرفوض',             bg: 'bg-red-500/10',     text: 'text-red-700',     icon: AlertTriangle },
  implemented:  { labelEn: 'Implemented',  labelAr: 'منفَّذ',             bg: 'bg-blue-500/10',    text: 'text-blue-700',    icon: BarChart2    },
}

const VOTE_ICONS: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
  for:     { icon: ThumbsUp,   color: 'text-emerald-700', bg: 'bg-emerald-500/10' },
  against: { icon: ThumbsDown, color: 'text-red-700',     bg: 'bg-red-500/10'     },
  abstain: { icon: Minus,      color: 'text-amber-700',   bg: 'bg-amber-500/10'   },
  pending: { icon: Clock,      color: 'text-[var(--color-text-muted)]', bg: 'bg-gray-100' },
}
...
export function DecisionsView() {
  ...
  const STATS = [
    { label: T.totalDecisions, value: '31', icon: Gavel,         color: 'text-blue-600',    bg: 'bg-blue-500/10',    change: T.thisMonth    },
    { label: T.pendingVote,    value: '4',  icon: Vote,          color: 'text-amber-600',   bg: 'bg-amber-500/10',   change: T.closingToday },
    { label: T.approved,       value: '19', icon: CheckCircle2,  color: 'text-emerald-600', bg: 'bg-emerald-500/10', change: T.approvalRate },
    { label: T.overdue,        value: '2',  icon: AlertTriangle, color: 'text-red-600',     bg: 'bg-red-500/10',     change: T.actionNeeded },
  ]

  const TABS = [
    { key: 'decisions',      label: T.allDecisions, badge: null as number | null },
    { key: 'voting',         label: T.votingCenter, badge: DECISIONS.filter((d) => d.status === 'pending_vote').length },
    { key: 'implementation', label: T.implTracker,  badge: null },
  ]

  const filtered = DECISIONS.filter((d) => {
    const matchSearch = d.title.toLowerCase().includes(search.toLowerCase()) || d.committee.toLowerCase().includes(search.toLowerCase())
      ...
      <div>
        <div className="flex items-center justify-between gap-4 flex-wrap mb-5">
          <div className="flex gap-1 p-1 rounded-xl bg-gray-100/60 flex-wrap">
            {TABS.map((tab) => (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-all duration-150 flex items-center gap-1.5 ${activeTab === tab.key ? 'bg-[var(--color-surface-elevated)] shadow-sm text-[var(--color-text)]' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text)]'}`}>
                {tab.label}
                {tab.badge != null && (
                  <span className="bg-amber-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{tab.badge}</span>
                )}
              </button>
            ))}
          </div>
          <button className="flex items-center gap-1.5 text-[12px] bg-[var(--color-brand)] text-white px-3.5 py-2 rounded-xl hover:bg-[var(--color-brand2)] transition-all font-medium shadow-sm">
            <Plus className="w-3.5 h-3.5" />{T.newDecision}
          </button>
        </div>
          ...
          <div className="space-y-3">
            <div className="rounded-2xl border border-[var(--color-border)]/50 bg-[var(--color-surface-elevated)] shadow-sm p-4 flex flex-wrap gap-3 items-center">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={T.searchDecisions}
              ...
              <span className="text-[11px] text-[var(--color-text-muted)] ml-auto">{filtered.length} {T.decisions}</span>
            </div>
            ...
            {filtered.map((decision) => {
              ...
              return (
                <div key={decision.id}
                  className={`rounded-2xl border bg-[var(--color-surface-elevated)] shadow-sm transition-all duration-150 ${decision.status === 'pending_vote' ? 'border-amber-200/60' : 'border-[var(--color-border)]/50'}`}>
                  <button onClick={() => setExpanded(isExpanded ? null : decision.id)}
                    className="w-full p-5 flex items-center gap-4 text-left hover:bg-gray-100/20 transition-colors rounded-2xl">
                    <div className={`p-2.5 rounded-xl shrink-0 ${cfg.bg}`}><CfgIcon className={`w-4 h-4 ${cfg.text}`} /></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-[14px] font-bold text-[var(--color-text)] leading-snug">{isRtl ? decision.titleAr : decision.title}</h3>
                          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                            <span className="flex items-center gap-1 text-[11px] text-[var(--color-text-muted)]"><Users2 className="w-3 h-3" />{decision.committee}</span>
                            <span className="flex items-center gap-1 text-[11px] text-[var(--color-text-muted)]"><Calendar className="w-3 h-3" />{decision.date}</span>
                      ...
                      {decision.votes.total > 0 && decision.status !== 'pending_vote' && (
                        <div className="mt-3 flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-[var(--color-text-muted)] shrink-0">{forPct}{T.forPctLabel}</span>
                        </div>
                      )}
                    </div>
                  </button>
                  ...
                  {isExpanded && (
                    <div className="border-t border-[var(--color-border)]/50 p-5 bg-gray-100/10">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                          <div>
                            <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-1.5">{T.decisionSummary}</p>
                            <p className="text-[13px] text-[var(--color-text)] leading-relaxed">{isRtl ? decision.descAr : decision.description}</p>
                          </div>
                          {decision.linkedTask && (
                            <div>
                              <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-1.5">{T.linkedTask}</p>
                              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-gray-100/50 border border-[var(--color-border)]/50">
                                <Link2 className="w-3.5 h-3.5 text-[var(--color-brand)]" />
                                <span className="text-[12px] text-[var(--color-text)]">{isRtl ? decision.linkedTaskAr : decision.linkedTask}</span>
                              </div>
                            </div>
                          )}
                          <div>
                            <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-1.5">{T.executiveAction}</p>
                            <p className="text-[12px] text-[var(--color-text-muted)] leading-relaxed p-2.5 rounded-xl bg-[var(--color-brand)]/5 border border-[var(--color-brand)]/20">
                              {isRtl ? decision.execActionAr : decision.executiveAction}
                            </p>
                          </div>
                        </div>
                        <div className="space-y-4">
                          {decision.votes.total > 0 && (
                            <div>
                              <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-2">{T.voteTally} ({decision.votes.total})</p>
                              <div className="grid grid-cols-3 gap-2">
                                {[
                                  ...
                                  <div key={v.key} className={`p-3 rounded-xl ${v.bg} text-center`}>
                                    ...
                                    <p className="text-[10px] text-[var(--color-text-muted)] font-semibold">{v.label}</p>
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          <div>
                            <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-2">{T.memberVotes}</p>
                            <div className="space-y-1.5">
                              {decision.members.map((m, i) => {
                                const vc = VOTE_ICONS[m.vote]
                                const VcIcon = vc.icon
                                return (
                                  <div key={i} className="flex items-center justify-between px-3 py-2 rounded-xl bg-[var(--color-surface-elevated)] border border-[var(--color-border)]/50">
                                    <div className="flex items-center gap-2">
                                      <div className="w-6 h-6 rounded-full bg-[var(--color-brand)]/10 flex items-center justify-center text-[9px] font-bold text-[var(--color-brand)]">
                                        {m.name.split('. ')[0]}
                                      </div>
                                      <span className="text-[12px] font-medium text-[var(--color-text)]">{m.name}</span>
                                    </div>
                                    <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${vc.bg} ${vc.color}`}>
                                      ...
                                      <span>{m.vote === 'for' ? T.forLabel : m.vote === 'against' ? T.againstLabel : m.vote === 'abstain' ? T.abstainLabel : (isRtl ? 'معلق' : 'Pending')}</span>
                                    </div>
                                  </div>
                                )
                              })}
        ...
        {activeTab === 'voting' && (
          <div className="space-y-4">
            ...
            {DECISIONS.filter((d) => d.status === 'pending_vote').map((decision) => {
              ...
              return (
                <div key={decision.id} className="rounded-2xl border border-amber-200/60 bg-[var(--color-surface-elevated)] shadow-sm p-6">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[14px] font-bold text-[var(--color-text)]">{isRtl ? decision.titleAr : decision.title}</h3>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[11px] text-[var(--color-text-muted)] flex items-center gap-1"><Users2 className="w-3 h-3" />{decision.committee}</span>
                        <span className="text-[11px] text-[var(--color-text-muted)] flex items-center gap-1"><Calendar className="w-3 h-3" />{T.deadline} {decision.votingDeadline}</span>
                      </div>
                    </div>
                    {myVote && (
                      <span className={`text-[10px] font-semibold rounded-lg px-2.5 py-1 ${VOTE_ICONS[myVote].bg} ${VOTE_ICONS[myVote].color}`}>
                        {T.voted} {myVote === 'for' ? T.forLabel : myVote === 'against' ? T.againstLabel : T.abstainLabel}
                      </span>
                    )}
                  </div>
                  <p className="text-[13px] text-[var(--color-text-muted)] leading-relaxed mb-4">{isRtl ? decision.descAr : decision.description}</p>
                  {!myVote ? (
                    <div className="flex items-center gap-3 flex-wrap">
                      <button onClick={() => handleVote(decision.id, 'for')}
                        className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/10 text-emerald-700 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all text-[13px] font-semibold">
                        <ThumbsUp className="w-4 h-4" />{T.voteFor}
                      </button>
                      <button onClick={() => handleVote(decision.id, 'against')}
                      ...
                      <button onClick={() => handleVote(decision.id, 'abstain')}
                        className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-100 border border-[var(--color-border)] hover:bg-gray-100/70 transition-all text-[13px] font-semibold text-[var(--color-text-muted)]">
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-[13px] font-semibold ${VOTE_ICONS[myVote].bg} ${VOTE_ICONS[myVote].color} border-current/20`}>
                        ...
                        {T.voteRecorded}{' '}
                        <span className="font-bold">{myVote === 'for' ? T.forLabel : myVote === 'against' ? T.againstLabel : T.abstainLabel}</span>
                      </div>
                      <button
        ...
        {activeTab === 'implementation' && (
          <div className="space-y-3">
            <p className="text-[13px] text-[var(--color-text-muted)]">{T.implSubtitle}</p>
            {DECISIONS.filter((d) => d.status === 'approved' || d.status === 'implemented').map((decision) => {
              ...
              const cfg = STATUS_CONFIG[decision.status]
              return (
                <div key={decision.id} className="rounded-2xl border border-[var(--color-border)]/50 bg-[var(--color-surface-elevated)] shadow-sm p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[14px] font-bold text-[var(--color-text)]">{isRtl ? decision.titleAr : decision.title}</h3>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[11px] text-[var(--color-text-muted)] flex items-center gap-1"><Users2 className="w-3 h-3" />{decision.committee}</span>
                        <span className="text-[11px] text-[var(--color-text-muted)] flex items-center gap-1"><Gavel className="w-3 h-3" />{T.decided} {decision.date}</span>
                      </div>
                    </div>
                    <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${cfg.bg} ${cfg.text}`}>{isRtl ? cfg.labelAr : cfg.labelEn}</span>
                  </div>
                  <div className="mt-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[11px] text-[var(--color-text-muted)]">{T.implProgress}</span>
                      <span className="text-[11px] font-bold text-[var(--color-text)]">{implementationPct}%</span>
                    </div>
                    <div className="h-2 rounded-full bg-gray-200 overflow-hidden">
                      <div className="h-full rounded-full bg-[var(--color-brand)]" style={{ width: `${implementationPct}%` }} />
                    </div>
                  </div>
                  <div className="mt-3 p-2.5 rounded-xl bg-[var(--color-brand)]/5 border border-[var(--color-brand)]/15">
                    <p className="text-[11px] text-[var(--color-text-muted)] leading-relaxed">
                      <span className="font-semibold text-[var(--color-text)]">{T.action}</span>
                      {isRtl ? decision.execActionAr : decision.executiveAction}
                    </p>
                  </div>
                  {decision.linkedTask && (
                    <div className="mt-2 flex items-center gap-2 text-[11px] text-[var(--color-text-muted)]">
                      ...
                      <span className="text-[var(--color-brand)] font-medium">{isRtl ? decision.linkedTaskAr : decision.linkedTask}</span>
                    </div>
                  )}
                </div>
              )
